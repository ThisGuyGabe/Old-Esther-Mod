    ...
    public class SantasLittleHelper : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            for (int i = 0; i < 1; i++)
            {
                ...
                newVelocity *= -1;
                Projectile.NewProjectile(source, new Vector2(position.X + Main.rand.Next(-1, 1), player.Center.Y - 600 + Main.rand.Next(-1, 1)), newVelocity, type, damage, knockback, player.whoAmI);
